...
public:
    ...
    void onEmgData(myo::Myo* myo, const int8_t* emg)
    {
		...
		emgFile.open(emgFileString.str(), std::ios_base::out);
	...
	void onOrientationData(myo::Myo *myo, const myo::Quaternion< float > &rotation) {
		...
		orientationFile.open(orientationFileString.str(), std::ios_base::out);
		orientationFile
			<< ',' << rotation.x()
			<< ',' << rotation.y()
			<< ',' << rotation.z()
		...
		orientationEulerFile.open(orientationEulerFileString.str(), std::ios_base::out);
		orientationEulerFile
			<< ',' << roll
			<< ',' << pitch
			<< ',' << yaw;
			//<< std::endl;
	...
	void onAccelerometerData(myo::Myo *myo, const myo::Vector3< float > &accel) {
		...
		accelerometerFile.open(accelerometerFileString.str(), std::ios_base::out);
		printVector(accelerometerFile, accel);
		accelerometerFile.close();
	}
	...
	void onGyroscopeData(myo::Myo *myo, const myo::Vector3< float > &gyro) {
		...
		gyroFile.open(gyroFileString.str(), std::ios_base::out);
		printVector(gyroFile, gyro);
		gyroFile.close();
	}
