...
String[][] acclines; // reads .csv file input (not functional yet)
String[][] orelines;

// cur accel / orientation reading (to be updated in each loops)
 String[][] accdims; // x/y/z accel values for myo1 (idx 0) & myo2 (idx 1)
 String[][] oredims;
...
void draw() {
  ...
  if (!readFromExcel) {
    ...
    String[] allData = split(inputCSVLines[i], ",");
    
    if (allData == null) {
      exit();
    } else {
    // myo 1
     dataCategory[0][0] = allData[0]; // acceleration data 
      dataCategory[0][1] = allData[1]; // orienation data 
      dataCategory[0][2] = allData[2]; //gyro data 
      dataCategory[0][3] = allData[3];; // emg data 
   
   // myo 2
      dataCategory[1][0] = allData[4]; // acceleration data 
      dataCategory[1][1] = allData[5]; // orienation data 
      dataCategory[1][2] = allData[6]; //gyro data 
      dataCategory[1][3] = allData[7];; // emg data 
    }
